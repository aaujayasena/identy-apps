 ...
 */
export const resolveUserDisplayName = (profileInfo, authState, fallback = null) => {
 ...
 * @return {string}
 */
export const resolveUsername = (username, userStoreDomain) => {
 ...
 * @return {string}
 */
export const resolveUserStoreEmbeddedUsername = (username) => {
 ...
 */
export const resolveUserEmails = (emails) => {
    if (!emails || !Array.isArray(emails) || emails.length < 1) {
        return [];
    }
    return emails.map((email) => {
        if (typeof email !== "string" && (email === null || email === void 0 ? void 0 : email.value)) {
            return email.value;
        }
        return email;
    });
